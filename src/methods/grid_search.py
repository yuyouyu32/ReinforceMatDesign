...
if __name__ == '__main__':
    ...
    result_df.to_excel('../results/random_search_result.xlsx', idnex=False)
    result_df = pd.read_excel('../results/random_search_result.xlsx')
    # 筛选 Dmax(mm): 5-20 ; yield(MPa): 1500-2000; E(%):10-35%的数据
    result_df = result_df[(result_df['Dmax(mm)'] >= 5) & (result_df['Dmax(mm)'] <= 20) & (result_df['yield(MPa)'] >= 1500) & (result_df['yield(MPa)'] <= 2200) & (result_df['Ε(%)'] >= 15) & (result_df['Ε(%)'] <= 35)]
    result_df.to_excel('../results/random_search_result_filtered.xlsx', index=False)
    ...
    result_df.to_excel('../results/random_search_result_filtered_similar.xlsx', index=False)