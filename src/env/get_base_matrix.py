...
def get_row_matrix(df: pd.DataFrame):
    ...
    df['element_base'] = df.apply(lambda row: row[row == max_values[row.name]].index.tolist(), axis=1)
    df_expanded = df.explode('element_base')
    return df_expanded
    ...
    for name, group in grouped:
        ...
        for column in group_data.columns:
            ...
            if min_val > 0:
                ...
                possibly_present[column] = (0.0, float(max_val) * 1.2 / 100)
...
def main():
    ...
    df = df.drop(columns=DropColumns).drop(columns=TargetColumns)
